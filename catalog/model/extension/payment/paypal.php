...
class ModelExtensionPaymentPayPal extends Model {
	...
	public function getOrderRecurrings() {
		$query = $this->db->query("SELECT `or`.`order_recurring_id` FROM `" . DB_PREFIX . "order_recurring` `or` JOIN `" . DB_PREFIX . "order` `o` USING(`order_id`) WHERE `or`.`status` = '1' AND `o`.`payment_code` = 'paypal'");
