...
class ControllerModulePayPalSmartButton extends Controller {
	...
	public function index() {		
		if ($this->config->get('paypal_status') && $this->config->get('paypal_client_id') && $this->config->get('paypal_secret') && isset($this->request->get['route'])) {						
			$status = false;
			...
			if ($status) {				
				...
				} elseif (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/paypal_smart_button.tpl')) {
					return $this->load->view($this->config->get('config_template') . '/template/module/paypal_smart_button.tpl', $data);
				} else {
					return $this->load->view('default/template/module/paypal_smart_button.tpl', $data);
				}
			}
		}
	}
	...
	public function createOrder() {
		...
		if (isset($this->request->post['product_id'])) {
			...
			if ($product_info) {
				...
				if (!$errors) {					
					if (!$this->model_module_paypal_smart_button->hasProductInCart($this->request->post['product_id'], $option, $recurring_id)) {
		...
		if (!$errors) {					
			...
			if ($paypal->hasErrors()) {
				...
				foreach ($errors as $error) {
					...
					$this->model_module_paypal_smart_button->log($error, $error['message']);
				}
			...
			if ($this->error && isset($this->error['warning'])) {
				$this->error['warning'] .= ' ' . sprintf($this->language->get('error_payment'), $this->url->link('information/contact', '', 'SSL'));
			}
						
			$data['paypal_order_id'] = '';
		
			if (isset($result['id']) && isset($result['status']) && !$this->error) {
				$this->model_module_paypal_smart_button->log($result, 'Create Order');
			
				if ($result['status'] == 'VOIDED') {
					$this->error['warning'] = sprintf($this->language->get('error_order_voided'), $this->url->link('information/contact', '', 'SSL'));
				}
			
				if ($result['status'] == 'COMPLETED') {
					$this->error['warning'] = sprintf($this->language->get('error_order_completed'), $this->url->link('information/contact', '', 'SSL'));
				}
		...
		if ($paypal_order_info && !$this->error) {
			...
			if ($this->customer->isLogged() && $this->customer->getAddressId()) {
				...
				if (isset($paypal_order_info['payer']['address']['country_code'])) {
					$country_info = $this->model_module_paypal_smart_button->getCountryByCode($paypal_order_info['payer']['address']['country_code']);
			...
			if ($this->cart->hasShipping()) {
				if ($this->customer->isLogged() && $this->customer->getAddressId()) {
					...
					if (isset($paypal_order_info['purchase_units'][0]['shipping']['address']['country_code'])) {
						$country_info = $this->model_module_paypal_smart_button->getCountryByCode($paypal_order_info['purchase_units'][0]['shipping']['address']['country_code']);
						...
						if ($country_info) {
							...
							if (isset($paypal_order_info['purchase_units'][0]['shipping']['address']['admin_area_1'])) {
								$zone_info = $this->model_module_paypal_smart_button->getZoneByCode($country_info['country_id'], $paypal_order_info['purchase_units'][0]['shipping']['address']['admin_area_1']);
	...
	public function confirmOrder() {
		...
		foreach ($products as $product) {
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
				...
				$price = $this->currency->format($unit_price, $currency_code);
				$total = $this->currency->format($unit_price * $product['quantity'], $currency_code);
			} else {
				$price = false;
				$total = false;
			}
			...
			if ($product['recurring']) {
				...
				if ($product['recurring']['trial']) {
					$recurring = sprintf($this->language->get('text_trial_description'), $this->currency->format($this->tax->calculate($product['recurring']['trial_price'] * $product['quantity'], $product['tax_class_id'], $this->config->get('config_tax')), $currency_code), $product['recurring']['trial_cycle'], $frequencies[$product['recurring']['trial_frequency']], $product['recurring']['trial_duration']) . ' ';
				}

				if ($product['recurring']['duration']) {
					$recurring .= sprintf($this->language->get('text_payment_description'), $this->currency->format($this->tax->calculate($product['recurring']['price'] * $product['quantity'], $product['tax_class_id'], $this->config->get('config_tax')), $currency_code), $product['recurring']['cycle'], $frequencies[$product['recurring']['frequency']], $product['recurring']['duration']);
				} else {
					$recurring .= sprintf($this->language->get('text_payment_cancel'), $this->currency->format($this->tax->calculate($product['recurring']['price'] * $product['quantity'], $product['tax_class_id'], $this->config->get('config_tax')), $currency_code), $product['recurring']['cycle'], $frequencies[$product['recurring']['frequency']], $product['recurring']['duration']);
				}
			}
			...
			if (!empty($data['shipping_address'])) {
				...
				if (!empty($results)) {
					...
					if (!empty($quote_data)) {
						...
						$data['action_shipping'] = $this->url->link('module/paypal_smart_button/confirmShipping', '', 'SSL');
					} else {
						unset($this->session->data['shipping_methods']);
						unset($this->session->data['shipping_method']);
		...
		foreach ($totals as $total) {
			$data['totals'][] = array(
				'title' => $total['title'],
				'text'  => $this->currency->format($total['value'], $currency_code)
			);
		}

		$data['action_confirm'] = $this->url->link('module/paypal_smart_button/completeOrder', '', 'SSL');
		...
		if (VERSION >= '2.2.0.0') {
			$this->response->setOutput($this->load->view('module/paypal_smart_button/confirm', $data));
		} elseif (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/paypal_smart_button/confirm.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/module/paypal_smart_button/confirm.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/module/paypal_smart_button/confirm.tpl', $data));
		}
	}
	...
	public function completeOrder() {		
		...
		if (isset($this->session->data['paypal_order_id'])) {			
			...
			if ($paypal_order_id && !$this->error) {				
				...
				if (!$this->error) {				
					if ($transaction_method == 'authorize') {
						$this->model_module_paypal_smart_button->log($result, 'Authorize Order');
						...
						if (isset($result['purchase_units'][0]['payments']['authorizations'][0]['status']) && isset($result['purchase_units'][0]['payments']['authorizations'][0]['seller_protection']['status'])) {
							...
							if ($authorization_status == 'CAPTURED') {
								$this->error['warning'] = sprintf($this->language->get('error_authorization_captured'), $this->url->link('information/contact', '', 'SSL'));
							}
							...
							if ($authorization_status == 'EXPIRED') {
								$this->error['warning'] = sprintf($this->language->get('error_authorization_expired'), $this->url->link('information/contact', '', 'SSL'));
							}
						...
						$this->model_module_paypal_smart_button->log($result, 'Capture Order');
						...
						if (isset($result['purchase_units'][0]['payments']['captures'][0]['status']) && isset($result['purchase_units'][0]['payments']['captures'][0]['seller_protection']['status'])) {
							...
							if ($capture_status == 'FAILED') {
								$this->error['warning'] = sprintf($this->language->get('error_capture_failed'), $this->url->link('information/contact', '', 'SSL'));
							}
	...
	public function paymentAddress() {
		...
		if (VERSION >= '2.2.0.0') {
			$this->response->setOutput($this->load->view('module/paypal_smart_button/payment_address', $data));
		} elseif (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/paypal_smart_button/payment_address.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/module/paypal_smart_button/payment_address.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/module/paypal_smart_button/payment_address.tpl', $data));
		}
	}
	...
	public function shippingAddress() {
		...
		if (VERSION >= '2.2.0.0') {
			$this->response->setOutput($this->load->view('module/paypal_smart_button/shipping_address', $data));
		} elseif (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/paypal_smart_button/shipping_address.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/module/paypal_smart_button/shipping_address.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/module/paypal_smart_button/shipping_address.tpl', $data));
		}
	}
