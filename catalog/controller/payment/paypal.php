...
class ControllerPaymentPayPal extends Controller {
	...
	public function index() {
		if ($this->config->get('paypal_client_id') && $this->config->get('paypal_secret')) {
			...
			if (VERSION >= '2.2.0.0') {
				return $this->load->view('payment/paypal', $data);
			} elseif (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paypal.tpl')) {
	...
	public function createOrder() {					
		...
		if (isset($this->request->post['checkout']) && ($this->request->post['checkout'] == 'card')) {
			if (empty($setting['currency'][$currency_code]['card_status'])) {
				$currency_code = $this->config->get('paypal_card_currency_code');
				$currency_value = $this->config->get('paypal_card_currency_value');
			}
		}
