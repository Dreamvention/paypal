...
class ControllerPaymentPayPal extends Controller {
	...
	public function __construct($registry) {
		...
		if (version_compare((string)phpversion(), '7.1', '>=')) {
			ini_set('precision', 14);
			ini_set('serialize_precision', 14);
		}
		
		if (empty($this->config->get('paypal_version')) || (!empty($this->config->get('paypal_version')) && (version_compare((string)$this->config->get('paypal_version'), '3.1.4', '<')))) {
			$this->update();
		}
	}
	...
	public function modal() {
		...
		$this->response->setOutput($this->load->view('payment/paypal/paypal_modal', $data));
	}
	...
	public function getData() {
		...
		if ($this->config->get('paypal_status') && $this->config->get('paypal_client_id') && $this->config->get('paypal_secret') && $agree_status && !empty($this->request->post['page_code'])) {	
			...
			$country = $this->model_payment_paypal->getCountryByCode($setting['general']['country_code']);
			...
			if (($this->request->post['page_code'] == 'checkout') && ($this->cart->hasProducts() || !empty($this->session->data['vouchers']))) {
				...
				if ($setting['card']['status']) {										
					...
					if ($setting['general']['vault_status'] && $this->customer->isLogged()) {
						$card_customer_tokens = $this->model_payment_paypal->getPayPalCustomerTokens($this->customer->getId(), 'card');
				...
				if ($setting['fastlane']['status'] && ($setting['general']['country_code'] == 'US') && !$this->customer->isLogged()) {
					...
					$data['fastlane_html'] = $this->load->view('payment/paypal/fastlane_customer', $data);
				}
			...
			if ($setting['general']['vault_status'] && $this->customer->isLogged()) {
				$paypal_customer_token = $this->model_payment_paypal->getPayPalCustomerMainToken($this->customer->getId(), 'paypal');
		...
		if (!empty($this->request->post['page_code']) && !empty($this->request->post['payment_type'])) {
			...
			if (($page_code == 'product') && (!empty($this->request->post['product']['product_id']))) {
				...
				if ($product_info) {
					...
					if (!$errors) {					
						if (!$this->model_payment_paypal->hasProductInCart($product_id, $option, $recurring_id)) {
			...
			if (!$errors) {					
				...
				if ($setting['general']['vault_status'] && ($this->customer->isLogged() || $this->cart->hasRecurringProducts())) {
					if ($payment_method == 'paypal') {
						...
						if ($this->customer->isLogged()) {
							$paypal_customer_token = $this->model_payment_paypal->getPayPalCustomerMainToken($this->customer->getId(), $payment_method);
						}
					...
					if ($payment_method == 'card') {
						if (isset($this->request->post['index'])) {
							...
							$card_customer_tokens = $this->model_payment_paypal->getPayPalCustomerTokens($this->customer->getId(), $payment_method);
				...
				if ($payment_method) {
					...
					$paypal_order_info['payment_source'][$payment_method]['experience_context']['return_url'] = $this->url->link('payment/paypal', 'callback_token=' . $setting['general']['callback_token'], true);
				...
				if (isset($result['id']) && isset($result['status']) && !$this->error) {
					$this->model_payment_paypal->log($result, 'Create Order');
					...
					if (($result['status'] == 'COMPLETED') && !empty($paypal_order_info['payment_source']['card']['vault_id'])) {
						...
						if (isset($paypal_order_info['payment_source']['card']) && !$this->error) {
							$this->model_payment_paypal->log($paypal_order_info['payment_source']['card'], 'Card');
						...
						if (!$this->error) {				
							...
							if ($transaction_method == 'authorize') {
								$this->model_payment_paypal->log($result, 'Authorize Order');
								...
								if (isset($result['purchase_units'][0]['payments']['authorizations'][0]['status']) && isset($result['purchase_units'][0]['payments']['authorizations'][0]['seller_protection']['status'])) {
									...
									if (($authorization_status == 'CREATED') || ($authorization_status == 'DENIED') || ($authorization_status == 'PENDING')) {
										$this->model_payment_paypal->deletePayPalOrder($this->session->data['order_id']);
										...
										$this->model_payment_paypal->addPayPalOrder($paypal_order_data);
									
										if ($vault_id && $this->customer->isLogged()) {
											$customer_id = $this->customer->getId();
										
											$paypal_customer_token_info = $this->model_payment_paypal->getPayPalCustomerToken($customer_id, $payment_method, $vault_id);
											...
											if (!$paypal_customer_token_info) {
												...
												$this->model_payment_paypal->addPayPalCustomerToken($paypal_customer_token_data);
											}
										
											$this->model_payment_paypal->setPayPalCustomerMainToken($customer_id, $payment_method, $vault_id);
										}
									}
									...
									if (($authorization_status == 'CREATED') || ($authorization_status == 'PENDING')) {
										...
										foreach ($recurring_products as $recurring_product) {
											$this->model_payment_paypal->recurringPayment($recurring_product, $order_info, $paypal_order_data);
										} 
									}
								...
								$this->model_payment_paypal->log($result, 'Capture Order');
				...
				if ($paypal_order_info && !$this->error) {
					...
					if ($this->customer->isLogged() && $this->customer->getAddressId()) {
						...
						if (isset($paypal_order_info['payer']['address']['country_code'])) {
							$country_info = $this->model_payment_paypal->getCountryByCode($paypal_order_info['payer']['address']['country_code']);
					...
					if ($this->cart->hasShipping()) {
						if ($this->customer->isLogged() && $this->customer->getAddressId()) {
							...
							if (isset($paypal_order_info['purchase_units'][0]['shipping']['address']['country_code'])) {
								$country_info = $this->model_payment_paypal->getCountryByCode($paypal_order_info['purchase_units'][0]['shipping']['address']['country_code']);
								...
								if ($country_info) {
									...
									if (isset($paypal_order_info['purchase_units'][0]['shipping']['address']['admin_area_1'])) {
										$zone_info = $this->model_payment_paypal->getZoneByCode($country_info['country_id'], $paypal_order_info['purchase_units'][0]['shipping']['address']['admin_area_1']);
					...
					if (($payment_type == 'googlepay_button') && !empty($this->request->post['payment_data'])) {
						...
						if (isset($payment_data['paymentMethodData']['info']['billingAddress']['countryCode'])) {
							$country_info = $this->model_payment_paypal->getCountryByCode($payment_data['paymentMethodData']['info']['billingAddress']['countryCode']);
						...
						if ($this->cart->hasShipping()) {
							...
							if (isset($payment_data['shippingAddress']['countryCode'])) {
								$country_info = $this->model_payment_paypal->getCountryByCode($payment_data['shippingAddress']['countryCode']);
					...
					if (($payment_type == 'applepay_button') && !empty($this->request->post['payment_data'])) {
						...
						if (isset($payment_data['billingContact']['countryCode'])) {
							$country_info = $this->model_payment_paypal->getCountryByCode($payment_data['billingContact']['countryCode']);
						...
						if ($this->cart->hasShipping()) {						
							...
							if (isset($payment_data['shippingContact']['countryCode'])) {
								$country_info = $this->model_payment_paypal->getCountryByCode($payment_data['shippingContact']['countryCode']);
	...
	public function confirmOrder() {
		...
		if (isset($this->request->post['coupon']) && $this->validateCoupon()) {
			...
			$this->response->redirect($this->url->link('payment/paypal/confirmOrder', '', true));
		}
		...
		$data['breadcrumbs'][] = array(
			'text' => $text_title,
			'href' => $this->url->link('payment/paypal/confirmOrder', '', true)
		);
				...
				if (!empty($results)) {
					foreach ($results as $result) {
						if ($this->config->get($result['code'] . '_status')) {
							...
							$quote = $this->{'model_shipping_' . $result['code']}->getQuote($data['shipping_address']);
					...
					if (!empty($quote_data)) {
						...
						$data['action_shipping'] = $this->url->link('payment/paypal/confirmShipping', '', true);
					} else {
						unset($this->session->data['shipping_methods']);
						unset($this->session->data['shipping_method']);
		...
		$data['action_confirm'] = $this->url->link('payment/paypal/completeOrder', '', true);
		...
		$data['total_coupon'] = $this->load->controller('total/coupon');
		$data['total_voucher'] = $this->load->controller('total/voucher');
		$data['total_reward'] = $this->load->controller('total/reward');
		...
		$this->response->setOutput($this->load->view('payment/paypal/confirm', $data));
	}
						...
						if ($transaction_method == 'authorize') {
							...
							if (isset($result['purchase_units'][0]['payments']['authorizations'][0]['status']) && isset($result['purchase_units'][0]['payments']['authorizations'][0]['seller_protection']['status'])) {
								...
								if (($authorization_status == 'CREATED') || ($authorization_status == 'PENDING')) {
									...
									foreach ($recurring_products as $recurring_product) {
										$this->model_payment_paypal->recurringPayment($recurring_product, $order_data, $paypal_order_data);
									} 
								}
	...
	public function paymentAddress() {
		...
		$this->response->setOutput($this->load->view('payment/paypal/payment_address', $data));
	}
	...
	public function shippingAddress() {
		...
		$this->response->setOutput($this->load->view('payment/paypal/shipping_address', $data));
	}
		...
		if ($this->customer->isLogged() && isset($this->request->post['index'])) {
			...
			if (!empty($card_customer_tokens[$card_token_index]['vault_id'])) {
				...
				if ($result && !$this->error) {
					$this->model_payment_paypal->deletePayPalCustomerToken($this->customer->getId(), 'card', $vault_id);
	...
	public function fastlaneModal() {
		...
		$data['text_fastlane'] = $this->language->get('text_fastlane');
		$data['text_fastlane_shipping'] = $this->language->get('text_fastlane_shipping');
		$data['text_fastlane_payment'] = $this->language->get('text_fastlane_payment');
		
		$data['shipping_required'] = $this->cart->hasShipping();
								
		$this->response->setOutput($this->load->view('payment/paypal/fastlane_modal', $data));
	}
	...
	public function fastlaneShipping() {
		...
		if (!empty($this->request->post['authentication_state']) && ($this->request->post['authentication_state'] == 'succeeded') && !empty($this->request->post['profile_data'])) {
			...
			if (isset($profile_data['shippingAddress']['address']['countryCode'])) {
				$country_info = $this->model_payment_paypal->getCountryByCode($profile_data['shippingAddress']['address']['countryCode']);
				...
				if ($country_info) {
					...
					if (isset($profile_data['shippingAddress']['address']['adminArea1'])) {
						$zone_info = $this->model_payment_paypal->getZoneByCode($country_info['country_id'], $profile_data['shippingAddress']['address']['adminArea1']);
		...
		$this->response->setOutput($this->load->view('payment/paypal/fastlane_shipping', $data));
	}
	...
	public function fastlanePayment() {
		...
		$this->response->setOutput($this->load->view('payment/paypal/fastlane_payment', $data));
	}
		...
		if (!empty($this->request->post['payment_token'])) {			
			...
			if (!empty($this->request->post['payment_source'])) {		
				...
				if (isset($payment_source['card']['billingAddress']['countryCodeAlpha2'])) {
					...
					$country_info = $this->model_payment_paypal->getCountryByCode($payment_source['card']['billingAddress']['countryCodeAlpha2']);
					...
					if ($country_info) {
						...
						if (isset($payment_source['card']['billingAddress']['region'])) {
							$zone_info = $this->model_payment_paypal->getZoneByCode($country_info['country_id'], $payment_source['card']['billingAddress']['region']);
				...
				$data['url'] = $this->url->link('payment/paypal/completeOrder', '', true);
			}
		}
		...
		if (!empty($this->request->get['callback_token'])) {
			...
			if (hash_equals($setting['general']['callback_token'], $this->request->get['callback_token']) && !empty($this->session->data['order_id']) && !empty($this->session->data['paypal_order_id']) && isset($this->session->data['paypal_card_token_index'])) {
				...
				$data['breadcrumbs'][] = array(
					...
					'href' => $this->url->link('payment/paypal/callback', '', true)
				);
				...
				$this->response->setOutput($this->load->view('payment/paypal/failure', $data));
	...
	public function webhook() {
		if (!empty($this->request->get['webhook_token'])) {
			...
			if (hash_equals($setting['general']['webhook_token'], $this->request->get['webhook_token']) && !empty($webhook_info['id']) && !empty($webhook_info['event_type'])) {	
				...
				$this->model_payment_paypal->log($webhook_info, 'Webhook');
				...
				if (($webhook_event['event_type'] == 'VAULT.PAYMENT-TOKEN.CREATED') && !empty($webhook_info['resource']['id']) && !empty($webhook_info['resource']['customer']['id']) && !empty($webhook_event['resource']['metadata']['order_id']) && !$errors) {
					...
					$paypal_order_info = $this->model_payment_paypal->getPayPalOrderByPayPalOrderId($paypal_order_id);
	...
	public function cron() {
		if (!empty($this->request->get['cron_token'])) {
			...
			if (hash_equals($setting['general']['cron_token'], $this->request->get['cron_token'])) {
				...
				$this->model_payment_paypal->cronPayment();
	...
	public function extension_get_extensions_after($route, &$output, $data) {
		if ($this->config->get('paypal_status') && $this->config->get('paypal_client_id') && $this->config->get('paypal_secret')) {
			$type = $data;
			...
			if ($type == 'payment') {			
				...
				if ($setting['googlepay_button']['checkout']['status']) {
					$this->config->set('paypal_googlepay_status', 1);
				...
				if ($setting['fastlane']['status'] && ($setting['general']['country_code'] == 'US') && !$this->customer->isLogged()) {
					$this->config->set('paypal_fastlane_status', 1);
	...
	public function order_delete_order_before($route, $order_id) {
		...
		$this->model_payment_paypal->deleteOrderRecurring($order_id);
		$this->model_payment_paypal->deletePayPalOrder($order_id);
		$this->model_payment_paypal->deletePayPalOrderRecurring($order_id);
	}
