	...
	var initPayPalSDK = function() {
		...
		if (paypal_data['card_customer_tokens'] && $('#paypal_card_tokens').length && !$('#paypal_card_tokens_container').html()) {
			...
			$.each(paypal_data['card_customer_tokens'], function(index, card_customer_token) {
				html = '<div class="paypal-card-token"><div type="button" class="btn card-token-button"><i class="card-icon card-icon-' + card_customer_token['card_type'] + '"></i><span class="card-number">' + card_customer_token['card_number'] + '</span></div><div type="button" class="btn card-token-delete-button"><i class="fa fal fa-close"></i></div></div></div>';
